...
def test_infer_writes_prediction_raster(inference_model_fp: Path, tile_case: dict, tmp_path: Path):
...
@pytest.mark.parametrize("tile_case", _END_TO_END_INFERENCE_CASES, indirect=True)
def test_infer_defaults_output_driver_to_gtiff(inference_model_fp: Path, tile_case: dict, tmp_path: Path):
...
def test_infer_feather_tiling_on_fhimp_case(
    inference_model_fp: Path,
