...
# Read lowres geotransform and native bbox (no lowres clipping).
...
lowres_xmin="$(echo "${lowres_gt_json}" | jq -r ".cornerCoordinates.upperLeft[0]")"
lowres_ymax="$(echo "${lowres_gt_json}" | jq -r ".cornerCoordinates.upperLeft[1]")"
lowres_xmax="$(echo "${lowres_gt_json}" | jq -r ".cornerCoordinates.lowerRight[0]")"
lowres_ymin="$(echo "${lowres_gt_json}" | jq -r ".cornerCoordinates.lowerRight[1]")"
...
# 1) Write lowres on its native bbox/grid, while assigning CRS from the DEM.
gdalwarp -overwrite \
    -of "${gdal_driver}" \
    -ot "${gdal_dtype}" \
...
# 2) Clip hires depth to lowres bbox at fixed hires resolution with nearest-neighbor resampling.
gdalwarp -overwrite \
    -of "${gdal_driver}" \
    -ot "${gdal_dtype}" \
    -co "COMPRESS=${gdal_compress}" \
