...
# Run tutorial 4 from a temp sandbox and copy the executed notebook back into the docs tree.
#
# Usage:
# - `conda run -n dev bash docs/user/notebooks/tutorial_4.sh`
#
# This tutorial mirrors the tutorial_2 execution pattern but targets the
# extended install because it runs the built-in CostGrow model and compares it
# against ResUNet on the same sample rasters.
#
...
#
# Outputs:
# - Overwrites `tutorial_4.ipynb` with the freshly executed notebook.
...
notebook_fp="${script_dir}/tutorial_4.ipynb"
stage_dir="${FLOODSR_NOTEBOOK_STAGE_DIR:-${FLOODSR_NOTEBOOK_CACHE_DIR:-}}"
cleanup_stage=0
if [ -z "${stage_dir}" ]; then
    stage_dir="$(mktemp -d -t floodsr-tutorial_4-XXXXXX)"
    cleanup_stage=1
fi
...
if ! command -v jupyter >/dev/null 2>&1; then
    echo "[tutorial_4] error: 'jupyter' was not found on PATH" >&2
    echo "[tutorial_4] hint: run this from the notebook-capable extended environment, e.g. 'conda run -n dev bash docs/user/notebooks/tutorial_4.sh'" >&2
    exit 1
fi
...
echo "[tutorial_4] notebook_fp=${notebook_fp}"
echo "[tutorial_4] repo_root=${repo_root}"
echo "[tutorial_4] stage_dir=${stage_dir}"
echo "[tutorial_4] run_dir=${run_dir}"
echo "[tutorial_4] python=$(python -c 'import sys; print(sys.executable)')"
echo "[tutorial_4] jupyter=$(python -m jupyter --version | tr '\n' ' ' )"
echo "[tutorial_4] staging notebook execution in ${run_dir}"
...
cd "${run_dir}"
echo "[tutorial_4] executing notebook"
...
echo "[tutorial_4] refreshed source notebook without leaving side files in ${script_dir}"
