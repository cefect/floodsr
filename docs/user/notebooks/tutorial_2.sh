...
if ! command -v jupyter >/dev/null 2>&1; then
    echo "[tutorial_2] error: 'jupyter' was not found on PATH" >&2
    echo "[tutorial_2] hint: run this from the notebook-capable environment, e.g. 'conda run -n dev bash docs/user/notebooks/tutorial_2.sh'" >&2
    exit 1
fi
